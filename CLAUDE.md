...
- **UI**：WPF + MaterialDesign，MVVM 架构（已完成主框架与 Dashboard/Tasks/Logs/TaskEditor 的现代化布局重构，FTP 任务编辑支持测试连接与 10 秒超时反馈）。
...
10. 已完成：任务编辑器支持 FTP 测试连接，可在保存前验证认证信息与基础路径是否有效，并具备 10 秒超时反馈。
