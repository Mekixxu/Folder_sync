...
- **UI**：WPF + MaterialDesign，MVVM 架构（已完成主框架与 Dashboard/Tasks/Logs/TaskEditor 的现代化布局重构，FTP 任务编辑支持测试连接、10 秒超时反馈，并修复测试结果弹窗关闭后主窗口卡住的问题；分析结果会提示“源文件被过滤规则全部排除”的场景）。
...
14. 待增强：双向冲突策略可扩展（例如保留双版本、副本命名策略、交互式冲突处理）。
