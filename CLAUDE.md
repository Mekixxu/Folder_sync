...
- **UI**：WPF + MaterialDesign，MVVM 架构（已完成主框架与 Dashboard/Tasks/Logs/TaskEditor 的现代化布局重构，FTP 任务编辑支持测试连接、10 秒超时反馈，并修复测试结果弹窗关闭后主窗口卡住的问题；分析结果会提示“源文件被过滤规则全部排除”的场景；任务页分析与执行链路已移到后台线程，避免主窗口在 FTP 分析期间假死；任务页与分析结果窗口在“正在分析”阶段会展示当前文件，并按已分析/待分析数据量驱动进度条比例）。
...
26. 已完成：任务页批量分析卡片与分析结果窗口的“正在分析”遮罩都会显示当前正在处理的文件路径；分析进度条在已知总量后会按“已分析数据量 / 待分析数据量”实时更新，而不再只按任务个数推进。
